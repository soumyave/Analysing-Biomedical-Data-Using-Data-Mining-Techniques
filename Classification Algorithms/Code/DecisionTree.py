...
def print_tree(node, depth=0):
    if node.predictlabel == None:
        print('%s[X%d < %.3f]' % ((depth*' ', (node.attr_index), node.attr)))
        print_tree(node.left, depth+1)
        print_tree(node.right, depth+1)
    else:
        print('%s[%s]' % ((depth*' ', node.predictlabel)))
...
def k_fold_validation(data):
    ...
    for i in range(0,k_fold):
        ...
        predicted_class = decisionTreeAlgorithm(train,test)
