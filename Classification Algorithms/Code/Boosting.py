...
#input :Training data,test record list
#output :classified classes of all the TestRecords
def decisionTreeAlgorithm(train,test):
    global decisionTree
    decisionTree = constructDecisionTree(train)
    predictedClassList = []
    for testdata in test:
        predictedClassLabel = predictClass(decisionTree,testdata)
        predictedClassList.append(predictedClassLabel)
    return decisionTree,predictedClassList  
...
def initializeBoostingVariables(training):
    length = len(training)
    initialweight = 1/length
    weightsData = [initialweight for x in training]
    indexes = [i for i in range(0,length)]
    return weightsData,indexes

def calculateMisclassificationError(errorClasses,weights):
    misclassificationerror = 0.0
    numerator = 0.0
    denominator = 0.0
    for i in range(0,len(errorClasses)):
        numerator += errorClasses[i] * weights[i]
    denominator = sum(weights)      
    misclassificationerror = numerator/denominator
    return misclassificationerror

def boostingAlgorithm(trainingData,testdata):
    classifiers = []
    classifierWeights =[]
    weightedTestSamples = []
    T = 5
    trainLength = len(trainingData)
    weightsStore,indexStore = initializeBoostingVariables(trainingData)
   
    for i in range(0,T):
        boostingSample = []
        testSample = []
        totalWeight = sum(weightsStore)
        weightsStore = [w/totalWeight for w in weightsStore] 
        
        while len(boostingSample) != trainLength:
            pickIndex = rd.choices(indexStore,weightsStore)
            pickSample = trainingData[pickIndex[0]]
            boostingSample.append(pickSample)
        
            
        tree,predictedclass= decisionTreeAlgorithm(boostingSample,boostingSample)
        actualclass = [x[-1] for x in boostingSample]
        misclassifiedclass = [0 if x == y else 1 for x,y in zip(predictedclass,actualclass)]
        
        misError = calculateMisclassificationError(misclassifiedclass,weightsStore)
        if(misError <= 0.5):
            clsWeight = 0.5 * math.log((1 - misError)/misError)
            classifierWeights.append(clsWeight)
            for index in range(0,trainLength):
                if misclassifiedclass[index] == 1:
                    weightsStore[index] *= math.exp(clsWeight)
                else:
                    weightsStore[index] *= math.exp(-clsWeight)
            weightsStore = [w/sum(weightsStore) for w in weightsStore] 
            classifiers.append(tree)
        
        
            
        for l in range(0,len(testdata)):
            if predictClass(tree,testdata[l]) == 0:
                testSample.append(-1.0)
            else:
                testSample.append(1.0)
       
        
        weightedTestSamples.append([tst * clsWeight for tst in testSample])
        
   
                            
    return weightedTestSamples           
...
def k_fold_validation(data):
    ...
    for i in range(0,k_fold):
        ...
        testclass = boostingAlgorithm(train,test)
        
        temp = np.array(testclass,dtype = float)
        boostingtestclass = np.sum(temp, axis=0)
        
        boostingtestclass  = [1 if boostingtestclass[l] > 0.0 else 0 for l in range(len(boostingtestclass))]
          
    
        actualclass = [x[-1] for x in test]
      
        perfMetrics = PerformanceMeasure(boostingtestclass,actualclass)
...
def printPerformanceStatistics(metrics):
    ...
    for i in metrics:
        if i != "Average":
            tempDict = metrics.get(str(i))
            print("-----------------------------------------------")
            print("\nPerformance Measures for fold :",str(i))
            print("Average measures of k-fold validation")
            print("Accuracy    :", tempDict.get('Accuracy'))
            print("Precision   :", tempDict.get('Precision'))
            print("Recall      :", tempDict.get('Recall'))
            print("F-1 Measure :", tempDict.get('F1_Measure'))
